...
def test_tier3_ratioofsum_k3():
    spec = AttributeSpec(n_rows=2200, n_attrs=48, n_informative=20, seed=121)
    X, metadata, rng = generate_attributes(spec)
    cfg = Tier3Config(positive_rate=0.1, sigma_logit=0.5, k=3, n_features=18, spec="ratioofsum")

    outputs = generate_tier3(X, metadata.informative_indices, cfg, rng)
...
def test_tier3_ratioofsum_k4():
    spec = AttributeSpec(n_rows=2500, n_attrs=60, n_informative=24, seed=212)
    X, metadata, rng = generate_attributes(spec)
    cfg = Tier3Config(positive_rate=0.1, sigma_logit=0.45, k=4, n_features=20, spec="ratioofsum")
    ...
    for feature_def in outputs.feature_defs:
        ...
        numerator = feature_def.get("numerator", {})
        denominator = feature_def.get("denominator", {})
        assert len(numerator) in {1, 2}
        assert len(denominator) in {2, 3}
        assert len(numerator) + len(denominator) == cfg.k
        cv = feature_def.get("denominator_cv")
        assert cv is not None
        assert cv > 0.1
...
def test_tier3_sumproduct_k3():
    spec = AttributeSpec(n_rows=2400, n_attrs=50, n_informative=22, seed=303)
    X, metadata, rng = generate_attributes(spec)
    cfg = Tier3Config(positive_rate=0.1, sigma_logit=0.5, k=3, n_features=18, spec="sumproduct")
    ...
    for feature_def in outputs.feature_defs:
        ...
        assert structure == "sum_times_single"
...
def test_tier3_sumproduct_k4():
    spec = AttributeSpec(n_rows=2600, n_attrs=60, n_informative=24, seed=404)
    X, metadata, rng = generate_attributes(spec)
    cfg = Tier3Config(positive_rate=0.1, sigma_logit=0.45, k=4, n_features=20, spec="sumproduct")
